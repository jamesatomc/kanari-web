...
export function VCSection({ darkMode, setDarkMode }: VCSectionProps) {
    // ข้อมูล VC (สามารถแก้ไขเพิ่ม/ลดได้ตามต้องการ)
    const vcs = [
        {
        ...
        if (!isPaused) {
            ...
            const scroll = () => {
                if (scrollContainerRef.current) {
                    ...
                    // เมื่อเลื่อนไปจนถึงครึ่งทาง (จุดจบของชุดแรก) ให้เด้งกลับมาเริ่มต้นใหม่แบบไร้รอยต่อ
                    if (container.scrollLeft >= container.scrollWidth / 2) {
                        container.scrollLeft = 0;
                    } else {
                        container.scrollLeft += 1; // ความเร็วในการเลื่อน (เพิ่มเลขเพื่อให้เร็วขึ้น)
                    }
                }
            ...
            <div className="max-w-7xl mx-auto">
                                ...
                                <div className="flex items-center mt-2">
                                    <div className="flex -space-x-2 mr-3">
                                        <div className="w-8 h-8 rounded-full ring-2 ring-white overflow-hidden">
                                            <Image src="https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=80&q=80&auto=format&fit=crop" alt="User 1" width={32} height={32} className="w-full h-full object-cover" />
                                        </div>
                                        <div className="w-8 h-8 rounded-full ring-2 ring-white overflow-hidden">
                                            <Image src="https://images.unsplash.com/photo-1545996124-1b5b7b4a8d06?w=80&q=80&auto=format&fit=crop" alt="User 2" width={32} height={32} className="w-full h-full object-cover" />
                                        </div>
                                        <div className="w-8 h-8 rounded-full ring-2 ring-white overflow-hidden">
                                            <Image src="https://images.unsplash.com/photo-1549221987-6f12d6b6d3a6?w=80&q=80&auto=format&fit=crop" alt="User 3" width={32} height={32} className="w-full h-full object-cover" />
                                        </div>
                                    </div>
                ...
                >
                    <div
                        ...
                        // ลบ style={{ scrollBehavior: 'smooth' }} ออกเพื่อให้ JS ทำงานได้ลื่น
                        className="flex space-x-6 overflow-x-hidden scrollbar-hide py-6 px-2"
                        role="list"
                        aria-label="Investor logos"
                    >
                        {/* นำ Array มาซ้ำ 2 รอบเพื่อให้เกิดการวนลูป (Infinite Loop) */}
                        {[...vcs, ...vcs].map((vc, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`vc-logo-pill flex items-center gap-3 px-6 py-3 rounded-full transition-transform duration-200 text-sm whitespace-nowrap cursor-pointer ${isSelected ? 'ring-2 ring-blue-500 shadow-lg scale-105 z-10' : 'hover:scale-105'} ${darkMode ? 'bg-white/6 border border-white/10' : 'bg-white/40 border border-gray-200'}`}
                ...
                {/* Modal สำหรับแสดงรายละเอียดเมื่อกดที่ VC */}
                {selectedVC !== null && (
                    <div
